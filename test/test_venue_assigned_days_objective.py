...
def test_venue_assigned_days_objective_sat() -> None:
...
def test_venue_assigned_days_objective_unsat_with_hard_constraint() -> None:
    constraints = [
        SingleFixtureDomainConstraint(),
...
def test_venue_assigned_days_objective_check_partial_score() -> None:
    """Multiple home fixtures on the same day reduce assigned-day utilization to a partial score."""
    league = _sat_league()
    fixtures = list(league.fixtures)
    fixtures[1].date = fixtures[0].date
    result = VenueAssignedDaysObjectiveConstraint().check(league)
